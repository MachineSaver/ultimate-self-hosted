...
generate_secrets() {
  ...
  JELLYFIN_OIDC_CLIENT_ID="jellyfin-$(gen_short_id)"
  JELLYFIN_OIDC_CLIENT_SECRET=$(gen_secret)
...
process_templates() {
  ...
  find config -name "*.template" | while read -r tmpl; do
    ...
    sed \
      ...
      -e "s|{{JELLYFIN_OIDC_CLIENT_ID}}|${JELLYFIN_OIDC_CLIENT_ID}|g" \
      -e "s|{{JELLYFIN_OIDC_CLIENT_SECRET}}|${JELLYFIN_OIDC_CLIENT_SECRET}|g" \
...
configure_services() {
  ...
  [[ "$all_ok" == "true" ]] \
    ...
    || die "Some configuration steps failed — see warnings above and re-run the failed scripts after fixing them"
}
...
verify_start_script() {
  step "Verifying scripts/start.sh"

  if [[ ! -x scripts/start.sh ]]; then
    die "scripts/start.sh is missing or not executable"
  fi
